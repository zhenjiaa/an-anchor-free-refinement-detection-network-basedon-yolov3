...
def test(data,
    ...
    all_img = 0
    det_img = [0]*10
    det_thre = [0.5,0.55,0.6,0.65,0.7,0.75,0.8,0.85,0.9,0.95]
            ...
            if nl:
                ...
                for cls in torch.unique(tcls_tensor):
                    ...
                    if pi.shape[0]:
                        ...
                        iii = box_iou(predn[pi, :4], tbox[ti])
                        all_img += iii.shape[1]
                        for th in range(len(det_thre)):
                            for iou_i in range(iii.shape[1]):
                                for iou_j in range(iii.shape[0]):
                                    if iii[iou_j][iou_i] >det_thre[th]:
                                        det_img[th] +=1
                                        break
                        print(det_img,all_img)
    ...
    #     print(i/all_img)
    #     all_det +=i
    # print(all_det/all_img)
    


if __name__ == '__main__':
    parser = argparse.ArgumentParser(prog='test.py')
    parser.add_argument('--weights', nargs='+', type=str, default='runs/train_1080p_res_free_03_03/exp/weights/best.pt', help='model.pt path(s)')
    parser.add_argument('--data', type=str, default='cfg/ccpd/box_1080p_size.yaml', help='*.data path')
